...
import { ArrowLeft, Heart, MessageSquare, Share2, ExternalLink, Github, Calendar } from 'lucide-react'
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {project.createdAt && (
              <div className="flex items-center text-sm text-gray-500 bg-gray-50 px-3 py-1 rounded-full">
                ...
                <span>Published on {formatDate(project.createdAt, true)}</span>
              </div>
            )}
          </div>
            ...
            {project.screenshots.map((screenshot, index) => (
              <div key={index} className="rounded-lg overflow-hidden border border-gray-200">
                <img 
                  ...
                  onError={(e) => {
                    // Fallback image if the screenshot fails to load
                    e.currentTarget.src = "https://images.unsplash.com/photo-1555421689-3f034debb7a6?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80";
                  }}
                />
              </div>
